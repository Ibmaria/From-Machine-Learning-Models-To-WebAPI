...
db = client.Monapi
utilisateurs = db["TeleMarcketAPI"]
...
class Predire_Deposit(Resource):
    def post(self):
        reponse_json = request.get_json()
        username = reponse_json["nom"]
        password = reponse_json["motdepasse"]
        age = reponse_json["age"]
        job = reponse_json["job"]
        education = reponse_json["education"]
        housing = reponse_json["housing"]
        pdays =reponse_json["pdays"]
        poutcome = reponse_json["poutcome"]
        cons_price_idx = reponse_json["cons_conf_idx"]
        euribor3m = reponse_json["euribor3m"]
        nr_employed =reponse_json["nr_employed"]
    
        
        names_col=['age','job','education','housing',
           'pdays','poutcome','cons_price_idx','euribor3m','nr_employed']
        
        df = pd.DataFrame(columns=names_col)
        row={"age":age,"job":job,
        "education":education,"housing":housing,"pdays":pdays,
        "poutcome":poutcome,"cons_price_idx":cons_price_idx,"euribor3m":euribor3m,
        "nr_employed":nr_employed}
        df=df.append(row, ignore_index=True)

        predictions=int(model.predict(df))
        print('*************************')
        print(predictions)
        if predictions ==1:
            output= 'Make Deposit'
...
api.add_resource(Predire_Deposit, '/classifier')
